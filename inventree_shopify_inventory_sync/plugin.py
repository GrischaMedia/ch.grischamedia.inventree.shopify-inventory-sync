...
class ShopifyInventorySyncPlugin(SettingsMixin, AppMixin, InvenTreePlugin):
    ...
    SETTINGS = {
        "shop_domain": {"name": "Shopify Shop Domain", "description": "z. B. my-shop.myshopify.com (ENV: SHOPIFY_SHOP_DOMAIN)", "default": "", "type": "string"},
        "admin_api_token": {"name": "Admin API Token", "description": "Shopify Admin API Access Token (ENV: SHOPIFY_ADMIN_API_TOKEN)", "default": "", "protected": True, "type": "string"},
        "use_graphql": {"name": "GraphQL verwenden", "description": "Für performantere SKU-Suchen", "default": True, "type": "boolean"},
        "inv_target_location": {"name": "InvenTree Ziel-Lagerort (ID)", "description": "ID des Lagerorts 'Onlineshop' (ENV: INVENTREE_TARGET_LOCATION_ID)", "default": "", "type": "string"},
    ...
    # Menü (optional; zeigt Links in der UI)
    def get_menu_items(self, request):
        if not (request.user.is_authenticated and _is_allowed(request.user)):
            return []
        return [
            {"name": "Shopify Sync – Übersicht", "link": reverse(f"plugin:{self.SLUG}-index"), "icon": "fa-external-link-alt"},
            {"name": "Shopify Sync jetzt (open)", "link": reverse(f"plugin:{self.SLUG}-sync-now-open"), "icon": "fa-sync"},
            {"name": "Shopify Sync jetzt", "link": reverse(f"plugin:{self.SLUG}-sync-now"), "icon": "fa-sync"},
        ]
